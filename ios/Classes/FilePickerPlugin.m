...
- (void) resolveMultiPickFromGallery:(MediaType)type withCompressionAllowed:(BOOL)allowCompression {
    ...
    // https://www.appsloveworld.com/swift/100/415/how-to-fix-navbar-uiimagepickercontroller-in-ios-15-be-transparent
    if (@available(iOS 13, *)) {
        UINavigationBarAppearance* appearance = [[UINavigationBarAppearance alloc] init];
        [appearance configureWithOpaqueBackground];
        appearance.backgroundColor = UIColorFromRGB(0x20202f);
        appearance.shadowColor = UIColorFromRGB(0x20202f);

        dkImagePickerController.navigationBar.standardAppearance = appearance;
        dkImagePickerController.navigationBar.scrollEdgeAppearance = appearance;
    }
...
-(void)picker:(PHPickerViewController *)picker didFinishPicking:(NSArray<PHPickerResult *> *)results API_AVAILABLE(ios(14)){
    ...
    for (PHPickerResult *result in results) {
        ...
        [result.itemProvider loadFileRepresentationForTypeIdentifier:@"public.item" completionHandler:^(NSURL * _Nullable url, NSError * _Nullable error) {
            ...
            NSString * extension = [FileUtils resolveImageExtension: url.path];
